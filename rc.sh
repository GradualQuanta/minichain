...
eval "$(ipms cat /ipfs/$envf)"

if false; then
 raw=https://raw.githubusercontent.com/Gradual-Quanta/minichain/master
 envf=$(bin/ipms add -Q $raw/.brings/minimal/envrc.sh --progress=0)
 if ipms file rm /.brings/envrc.sh then
  ipms file cp /ipfs/$envf /.brings/envrc.sh
 fi
